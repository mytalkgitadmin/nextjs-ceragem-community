...
export const useUIStore = (): UIState => {
  ...
  const openChannelDrawer = useChannelDrawerStore(
    (state) => state.openChannelDrawer
  );
  const closeChannelDrawer = useChannelDrawerStore(
    (state) => state.closeChannelDrawer
  );
