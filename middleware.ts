...
export async function middleware(req: NextRequest) {
  ...
  const coachConnected = req.cookies.get("coach_connected")?.value;

  console.log("[MW] pathname:", pathname, isHtml);
  ...
  if (coachConnected !== "true") {
    const url = new URL("/coach-connect", req.url);
    url.searchParams.set("to", req.nextUrl.toString());
    return NextResponse.rewrite(url);
  }
